...
HAL_Return_Type Func_set_MCAN0_N0_init_off (void)
{
    ...
    uint32 timeout = 100000U;   /* bounded wait */

       /* Wait until INIT is actually cleared or timeout */
           while ((CAN0_CCCR0.B.INIT == 1U) && (timeout > 0U))
           {
               timeout--;
           }
...
/*********************************************************************************************************************/
/*
 * The function Func_MCAN0_N0_TxBuffer0_Send_u16 sends a 16-bit value via MCAN0 Node0 using TX Buffer 0.
 *
 * It performs the following steps:
 * 1) Check whether TX Buffer 0 is currently pending (TRP0 == 0). If pending, the buffer is not overwritten.
 * 2) Write the 16-bit value into the first two data bytes of TX Buffer 0 in little-endian format.
 *    - Byte0 = low byte
 *    - Byte1 = high byte
 * 3) Trigger a transmission request for TX Buffer 0 by setting AR0.
 * 4) Verify that the MCAN module accepted the request (TRP0 becomes 1).
 *
 * Return value:
 * - HAL_E_OK      if the transmission request was accepted and the attempt started
 * - HAL_E_NOT_OK  if the buffer was still pending or the request was not accepted
 */
...
HAL_Return_Type Func_MCAN0_N0_TxBuffer0_Send_u16(uint16 value)
{
    ...
    if (CAN0_N0_TX_BRP.B.TRP0 == 0)
    {
        ...
        /* Check: did MCAN accept the request? */
        if (CAN0_N0_TX_BRP.B.TRP0 == 1)
         {
             tmp_HAL = HAL_E_OK;   /* Transmission attempt started */
         }
